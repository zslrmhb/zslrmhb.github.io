import { o as je, s as yt } from './C5HLncaA.js';
import { H as ce, S as Se, R as Ee } from './CYgJF_JY.js';
import { bu as Re, aM as T, C as x, aj as P, t as $e } from './7lH0bu5J.js';
...
const U = globalThis.__sveltekit_1ggrly5?.base ?? '',
	Ot = globalThis.__sveltekit_1ggrly5?.assets ?? U,
	Nt = '1777593723070',
...
function Me() {}
let Le, be, ne, C, ke, v;
globalThis.__sveltekit_1ggrly5.data;
const re = [],
	ae = [];
let O = null;
const Q = new Map(),
...
async function de({ status: e, error: t, url: n, route: r }) {
	...
	if (v.server_loads[0] === 0)
		try {
			...
			(n.origin !== le || n.pathname !== location.pathname || Te) && (await M(n));
		}
	try {
		const o = await Pe({
				loader: be,
...
async function W({
	...
	let _ = u && (await dt(u));
	if (!_) {
		if (ue(t, U, v.hash)) return await M(t);
		_ = await pt(
			t,
			{ id: null },
	...
	((history.scrollRestoration = 'manual'),
		...
		C.addEventListener('click', async (t) => {
			...
			if (d !== void 0 && h) {
				...
				if (((q = !0), Ue(k), e(r), !o.replace_state)) return;
				q = !1;
			}
			(t.preventDefault(),
				await new Promise((u) => {
		...
		addEventListener('hashchange', () => {
			q &&
				((q = !1), history.replaceState({ ...history.state, [F]: ++k, [Y]: I }, '', location.href));
		}));
